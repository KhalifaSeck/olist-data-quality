...
def check_payment_mismatch() -> pd.DataFrame:
    """
    Vérifie que le total des paiements correspond
    au total des items (insight du notebook).
    """
    query = """
        select
            p.order_id,
        ...
        group by p.order_id
        having abs(
            round(sum(p.payment_value) - sum(i.total), 2)
        ) > 0.01
    """
    df = read_sql(query)
...
def check_stale_shipped_orders() -> pd.DataFrame:
    """Commandes expédiées > 30j sans livraison confirmée."""
    query = """
        select
            order_id,
            order_status,
...
def check_low_review_categories() -> pd.DataFrame:
    """
    Catégories avec score moyen < 3.0
    (insight du notebook sur les mauvaises reviews).
    """
    query = """
        select
...
def check_repeat_purchase_rate() -> dict:
    """
    Calcule le taux de réachat (repeat purchase rate).
    Référence notebook : taux = 3% → problème de rétention.
    """
    query = """
        select
